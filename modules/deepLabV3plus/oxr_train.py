...
from datasets import OXRSegmentation
...
def get_argparser():
    ...
    # Datset Options
    parser.add_argument("--data_root", type=str, default='/home/workplace/HOnnotate_OXR/dataset/oxr_seg_data',
                        help="path to Dataset")
    parser.add_argument("--dataset", type=str, default='oxr',
                        choices=['voc', 'cityscapes', 'oxr'], help='Name of dataset')
...
def get_dataset(opts):
    if opts.dataset == "oxr":
        ...
        train_dst = OXRSegmentation(root=opts.data_root, image_set='train', transform=train_transform)
        val_dst = None
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            # targets = labels.detach().max(dim=1)[1].cpu().numpy()
...
def main():
    ...
    # train_dst, val_dst = data.random_split(dst, [int(len(dst) * 0.9), len(dst) - int(len(dst) * 0.9)])
    ...
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
        ...
        # print(model.classifier.classifier[3])
        ...
        raise ValueError("Can't find checkpoint {}".format(opts.ckpt))
        print("[!] Retrain")
        model = nn.DataParallel(model)
        model.to(device)
