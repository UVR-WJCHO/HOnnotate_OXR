...
def load_img_mano(img_path):
    ...
    # # 640 480 to 640, 640 with border
    # if img_w > img_h:
    #     # Pad top and bot
    #     padding_top = round((params.IMG_SIZE - img_h)/2)
    #     padding_bot = params.IMG_SIZE - img_h - padding_top
    # elif img_w < img_h:
    #     # Pad left and right
    #     padding_left = round((params.IMG_SIZE - img_w)/2)
    #     padding_right = params.IMG_SIZE - img_w - padding_left
    #
    # img_padded = cv2.copyMakeBorder(img_orig, top=padding_top, bottom=padding_bot, left=padding_left, \
    #     right=padding_right, borderType=cv2.BORDER_CONSTANT)
    # # For tkinter display
    # # img_padded = cv2.cvtColor(img_padded, cv2.COLOR_BGR2RGB)
    return img_orig, img_orig
