...
def CvCornerFinder(path, nsize, vis_mode):
    ...
    # retval, corners = cv2.findChessboardCornersSB(gray, nsize, flags=cv2.CALIB_CB_EXHAUSTIVE + cv2.CALIB_CB_ACCURACY)
...
def StereoCalibrate(imgDir, cam1, cam2, intrinsicMatrices, distCoeffs, imgInt=5, numImg=300, nsize=(12,7), minSize=30, vis_mode=False):
    ...
    for i in tqdm(range(imgInt-1, numImg, imgInt)):
        if not os.path.exists(os.path.join(rgbDir, cam1, f"{cam1}_{i}.jpg")):
            print("no img    ", os.path.join(rgbDir, cam1, f"{cam1}_{i}.jpg"))
            continue
        if not os.path.exists(os.path.join(rgbDir, cam2, f"{cam2}_{i}.jpg")):
            print("no img   ", os.path.join(rgbDir, cam2, f"{cam2}_{i}.jpg"))
            continue

        isValidLeft, leftCorners = CvCornerFinder(os.path.join(rgbDir, cam1, f"{cam1}_{i}.jpg"), nsize, vis_mode=vis_mode)
        isValidRight, rightCorners = CvCornerFinder(os.path.join(rgbDir, cam2, f"{cam2}_{i}.jpg"), nsize, vis_mode=vis_mode)
