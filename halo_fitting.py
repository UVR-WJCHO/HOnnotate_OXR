...
from modules.utils.haloUtils import preprocess_joints
from utils import *
...
sys.path.insert(0, 'dependencies/halo/halo_base')
from artihand import config, data
from artihand.checkpoints import CheckpointIO

sys.path.insert(0, 'dependencies/halo/halo/models')
from halo_adapter.transform_utils import xyz_to_xyz1

# ContactPose dependencies
sys.path.insert(0, 'dependencies/ContactPose')
...
## FLAGS
FLAGS = flags.FLAGS
flags.DEFINE_string('db', '230923', 'target db name')   ## name ,default, help
flags.DEFINE_string('cam_db', '230923_cam', 'target db name')   ## name ,default, help
...
def main(argv):
    ...
    for target_seq in seq_list:
        ...
        for trialIdx, trialName in enumerate(sorted(os.listdir(target_dir))):
            save_path = os.path.join(target_dir_result, trialName, "visualization_halo")
            ...
            for frame in range(len(mas_dataloader)):
                ...
                ''' HALO FITTING CODE SNIPPET '''
                try:
                    halo_kpts = mas_dataloader[frame]['kpts3d_halo'] / 1000
                    mano_kpts = mas_dataloader[frame]['kpts3d_mano'] / 1000
                except:
                    continue

                # MANO kpts conversion
                mano_fitter = MANOFitter()
                mano_params, verts, faces, joints = mano_fitter.fit_joints((halo_kpts.detach().cpu().numpy()))
                org_halo_kpts = halo_kpts = joints

                '''
                #vis_kpts = halo_kpts - halo_kpts[0, 0]
                #vis_kpts = vis_kpts.squeeze().detach().cpu().numpy()

                #vis_kpts_mano = mano_kpts - mano_kpts[0, 0]
                #vis_kpts_mano = vis_kpts_mano.squeeze().detach().cpu().numpy()

                import open3d as o3d
                pcd = o3d.geometry.PointCloud()
                pcd.points = o3d.utility.Vector3dVector(verts[0])
                o3d.io.write_point_cloud('verts.ply', pcd)

                #pcd.points = o3d.utility.Vector3dVector(halo_kpts.squeeze().detach().cpu().numpy())
                #o3d.io.write_point_cloud('kpts.ply', pcd)

                pcd.points = o3d.utility.Vector3dVector(joints)
                o3d.io.write_point_cloud('kpts_mano.ply', pcd)
                '''
                import open3d as o3d
                pcd = o3d.geometry.PointCloud()
                pcd.points = o3d.utility.Vector3dVector(halo_kpts*1000)
                o3d.io.write_point_cloud('kps.ply', pcd)

                '''
                pcd.points = o3d.utility.Vector3dVector(vis_kpts_mano)
                o3d.io.write_point_cloud('kps_mano.ply', pcd)
                ...
                halo_scale = LA.norm(halo_kpts[9] - halo_kpts[0])  
                scale = 0.1 / halo_scale

                halo_kpts *= scale
                
                mano_perm = [0, 17, 18, 19, 20, 1, 2, 3, 4, 5, 6, 7, 8, 13, 14, 15, 16, 9, 10, 11, 12]
                halo_kpts = torch.Tensor(halo_kpts[mano_perm]).unsqueeze(0).cuda()

                # preprocess joints
                data = {}
                inputs, bone_lengths, root_rot_mat = preprocess_joints(halo_kpts)
                data['inputs'] = inputs.unsqueeze(0)
                data['bone_lengths'] = bone_lengths.unsqueeze(0)
                data['root_rot_mat'] = root_rot_mat.unsqueeze(0)

                # load HALO model
                cfg = config.load_config('dependencies/halo/configs/halo_base/yt3d_b16_keypoint_normalized_fix.yaml', 'dependencies/halo/halo_base/configs/default.yaml')
                model = config.get_model(cfg, device='cuda')
                checkpoint_io = CheckpointIO('dependencies/halo/resource/halo_base_yt3d', model=model)
                checkpoint_io.load('model_best.pt')

                # load im2mesh generator
                ...
                verts = torch.matmul(root_rot_mat.squeeze().cpu().T, xyz_to_xyz1(torch.Tensor(verts)).unsqueeze(-1))[:, :3, 0] * 0.4
                verts = np.asarray(verts)
                verts += halo_kpts[0, 0].detach().cpu().numpy()
                verts /= scale
                verts /= 10
                verts *= 1000
                ...
                mesh.vertices = verts
                pred_hand = {'joints': torch.Tensor(org_halo_kpts).unsqueeze(0).cuda()*1000/10/scale, 'verts': torch.Tensor(verts).unsqueeze(0).cuda(), 'faces': torch.Tensor(np.asarray(mesh.faces)).unsqueeze(0).cuda()}
                print('detected cams')
                print(detected_cams)

                loss_func.visualize(pred=pred_hand, pred_obj=None, camIdxSet=detected_cams, frame=frame,
                                        save_path=save_path, flag_obj=False, flag_crop=True, flag_headless=FLAGS.headless)
                mesh.export(f'meshes/{frame}.ply')

                continue
                ''' END OF HALO FITTING CODE SNIPPET '''
