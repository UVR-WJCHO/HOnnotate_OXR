...
def load_annotation(targetDir, seq, trialName):
    ...
    for camID in cam_list:
        anno_folder = os.path.join(anno_base_path, camID)
        for anno_path in os.listdir(anno_folder):
            anno_path_full = os.path.join(anno_base_path, camID, anno_path)
            with open(anno_path_full, 'r', encoding='UTF-8 SIG') as file:   # encoding='cp949' for KAIST
                anno = json.load(file)
            break
    ...
    for camIdx, camID in enumerate(cam_list):
        ...
        for anno_path in os.listdir(anno_folder):
            file_name = anno_path[:-5]
            anno_path_full = os.path.join(anno_base_path, camID, file_name + '.json')
            ## s3 db path
            rgb_path_full = os.path.join(rgb_base_path, camID, file_name + '.jpg')
            depth_path_full = os.path.join(depth_base_path, camID, file_name + '.png')

            ## KAIST path
            # frame = int(anno_path[5:-5])
            # rgb_path_full = os.path.join(rgb_base_path, camID, f'{camID}_{frame}.jpg')
            # depth_path_full = os.path.join(depth_base_path, camID, f'{camID}_{frame}.png')
            ...
            rgb_input = np.asarray(cv2.imread(rgb_path_full))
            depth_input = np.asarray(cv2.imread(depth_path_full, cv2.IMREAD_UNCHANGED)).astype(float)
            ...
            ### output : rgb_2d_pred, hand_seg_masked, obj_seg_masked ###
