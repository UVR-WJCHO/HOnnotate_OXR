...
mainCamID = 1
mainCamName = camIDset[mainCamID]
    ...
    for i in range(numIter):
        ...
        for f in range(numFrames):
            if (i % 1 == 0) and FLAGS.showFig:
                ...
                ### for main cam, only intrinsic required ###
                ...
                cv2.imwrite(os.path.join(out_dir, str(f) + '_blend.png'), imgJoints)
...
def dataGen(w, h):
    configFile = join(OXR_MULTI_CAMERA_DIR, FLAGS.db, FLAGS.seq, 'configs/configHandPose.json')
    with open(configFile) as config_file:
        data = yaml.safe_load(config_file)
    ...
    imgColorDir = os.path.join(base_dir, 'rgb', camID)
    imgDepthDir = os.path.join(base_dir, 'depth', camID)
    
    # raw_seg_folder = os.path.join(base_dir, 'hand_only', 'mask')
    raw_seg_folder = os.path.join(base_dir, 'segmentation', camID, 'raw_seg_results')


    handSegDir = os.path.join(base_dir, 'segmentation', mainCamName, 'raw_seg_results')

    
    if datasetName == datasetType.OXR_MULTICAMERA:
        autoInitDir = join(base_dir, 'handInit', 'singleFrameMultiViewFit')
    ...
    if datasetName == datasetType.OXR_MULTICAMERA:
        dataset = datasetOXRMultiCamera(FLAGS.db, FLAGS.seq, mainCamName)
    elif datasetName == datasetType.HO3D_MULTICAMERA:
    ...
    for file in files:
        if datasetName == datasetType.OXR_MULTICAMERA:
            ImgID = FLAGS.seq + '/' + file            
            _, ds = dataset.createTFExample(itemType='hand', fileIn=ImgID)
            
            img = ds.imgRaw     # img = ds.imgRaw[:, :, [2, 1, 0]]
            dpt = ds.depth
            depthScale = ds.depthScale
            # reading the seg from file because the seg in ds has no objAliasLabel info
            seg = cv2.imread(join(handSegDir, file + '.png'))[:, :, 0]
            
            
            ########## segH is same as seg? it seems already annotated with 0, 1, 2 value
            segH = np.copy(seg)     
            # cv2.imread(join(handSegDir, file + '.png'))[:,:,0]
...
if __name__ == '__main__':
    ...
    ################# specify target object with config file #################
    configFile = join(OXR_MULTI_CAMERA_DIR, FLAGS.db, FLAGS.seq, 'configs/configHandPose.json')
    with open(configFile) as config_file:
        data = yaml.safe_load(config_file)
    base_dir = os.path.join(OXR_MULTI_CAMERA_DIR, FLAGS.db, FLAGS.seq)
    ...
    camID = mainCamName     # data['camID'] : mas, sub1, ...
        
    modelPath = os.path.join(YCB_MODELS_DIR, data['obj'])
    mesh = load_mesh(modelPath)
        
    ### specify hand shape parameter, does it needed?
    ...
    files = os.listdir(autoInitDir)
    files = [f[:-7]+'.jpg' for f in files if 'pickle' in f]
    ...
    ### check which beta to use
    # beta_pred = newInit['beta']
    print("beta shape : ", beta.shape)
    handParamAnchorFrame = handParams(theta=newFullpose, trans=newTrans, beta=beta, Scale=scaleConstr)




    ################# load anchor object pose from pkl #################
    if USE_DUMMY:
        newObjRot = np.zeros((3,), dtype=np.float32)
        newObjTrans = np.array([0., 0., -0.4])
    
    elif OBJ_POSE_FROM_DIRT:
    ...
    for i in range(len(files)):
        ...
        if use2DJointLoss:
            if datasetName == datasetType.OXR_MULTICAMERA:
                joints2dDir = join(base_dir, 'handInit', 'singleFrameMultiViewFit')
    ...
    for file in files:
        if USE_DUMMY:
            ...
            dummy_Trans = np.array([0., 0., -0.4])
            paramInit = objParams(rot=dummy_Rot, trans=dummy_Trans)
        else:
    ...
    if datasetName == datasetType.OXR_MULTICAMERA:
        camMat = datasetOXRMultiCamera.getCamMat(mainCamName)
        camProp = camProps(ID=mainCamName, f=np.array([camMat[0,0], camMat[1,1]], dtype=np.float32) / dscale,
